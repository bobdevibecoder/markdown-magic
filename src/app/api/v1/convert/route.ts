...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!to || !["html", "markdown"].includes(to)) {
      return NextResponse.json(
    ...
    let result: string;
    const direction: "md_to_html" | "html_to_md" = to === "csv" ? "md_to_html" : "html_to_md";

    if (to === "csv") {
      const input = typeof data === "string" ? data : JSON.stringify(data);
      result = convertForward(input);
    } else {
